    ...
    static IsTouchOverUI(pos: G.v2) {
        const checkTouchFunc = function (node: G.Ett, pos: G.v2) {
            ...
            if (!node || !node.active) {
                ...
                // check self
                const touchInputComponent = node.getComponent(G.E.TouchInputComponent) as G.TouchInputComponent;
                if (touchInputComponent) {
                    const canvasWorldPos = MGame.game.rootUICamera.convertEventPositionToUICanvas(pos);
